...
The current version is `v2`, the biggest difference of which to `v1` is to remove the method `GetKeys()` from the parser interface `Parser`. But you can use the branch [v1](https://github.com/xgfone/go-config/tree/v1). For the user, there is no effect. So you have no need to modify any code, and just the parser plugin should be modified.
